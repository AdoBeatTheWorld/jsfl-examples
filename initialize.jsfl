...
function parse()
{
	...
	for(j = 0; j < len; j++)
	{
		...
		}else if(target.parameters)
		{
			...
			//遍历他的parameters看看有没有cmdId这个字段，有的话更新，没有的话插入
			for(o = 0; o < para_len; o++)
			{
				if(target.parameters[o].name=="cmdId")
				{
					target.parameters[o].value=Number(children[j].@cmdId);
					flag = true;
					//fl.trace("update cmdId to "+children[j].@cmdId);
				}
			}
			if(flag == false)
			{
				target.parameters[0].insertItem(0,"cmdId",Number(children[j].@cmdId),"Number");
				//fl.trace("insert cmdId "+children[j].@cmdId);
			}
			flag = false;
