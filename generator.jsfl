...
for(i = 0; i < list.length;i++)
{
	...
	//如果是导出的面板
	if(list[i].linkageBaseClass == "flash.display.Sprite" || list[i].linkageBaseClass == "flash.display.MovieClip"){
		/*var path = arr[0]+"_"+list[i].name+".swf";
		path = FLfile.platformPathToURI(path);
		list[i].exportSWF(path);*/
		parseItem(list[i],"");
	}
}
	...
	for(j = layerIndex; j < len1; j++)
	{
		...
		for(k = frameIndex; k < len2; k++)
		{
			...
			for(m = elementIndex; m < _elements.length; m++)
			{
				...
				//检查他是否有cmdId这个属性
				var cmdId = checkCmdid(_element.parameters);
				var elementNode = XML("<element />");
				
				if(cmdId == null){
					continue;
				}
				elementNode.@cmdId = cmdId;
